#!/usr/bin/env python3
"""
Heavy Bag Boxing Trainer - Detects heavy bag and only counts punches on contact
Eliminates false positives by requiring hand contact with bag.
"""
...
class HeavyBagTrainer:
    """Heavy bag trainer with bag detection and contact-based counting."""
    ...
    def __init__(self, camera_id: int = 0):
        ...
        self.min_frames_before_detection = 120  # 4 seconds for bag detection
        ...
        self.contact_threshold = 50  # pixels - how close hand must be to bag
        
        print("Heavy Bag Trainer initialized")
        print("Will detect heavy bag and only count punches on contact")
        ...
        if self.frames_processed >= self.min_frames_before_detection:
            if self.setup_phase:
                ...
                if self.bag_detected:
                    ...
                    print("SETUP COMPLETE - No heavy bag detected. Switching to regular detection.")
    ...
    def draw_camera_view(self, frame: np.ndarray, landmarks_dict: dict) -> np.ndarray:
        """Draw camera view with bag detection overlay."""
        height, width = frame.shape[:2]
        
        # Header
        cv2.rectangle(frame, (0, 0), (width, 80), (0, 0, 0), -1)
        cv2.putText(frame, 'HEAVY BAG MODE', (20, 30), 
                   cv2.FONT_HERSHEY_SIMPLEX, 1.0, (255, 0, 255), 2)
        
        # Bag detection status
        if self.bag_detected:
            cv2.putText(frame, 'BAG DETECTED', (20, 60), 
            ...
            if self.bag_area:
                ...
                cv2.putText(frame, 'HEAVY BAG', (x, y - 10), 
                           cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
        else:
            cv2.putText(frame, 'SCANNING FOR BAG...', (20, 60), 
        ...
        if landmarks_dict and self.bag_detected:
            for arm in ['left', 'right']:
                ...
                if wrist_key in landmarks_dict:
                    ...
                    cv2.putText(frame, arm.upper(), (hand_pos[0] + 15, hand_pos[1]), 
    ...
    def create_data_window(self) -> np.ndarray:
        ...
        # Title
        cv2.putText(data_window, 'HEAVY BAG TRAINER - CONTACT REQUIRED', (150, 60), 
        ...
        cv2.putText(data_window, 'Only counts punches that contact the heavy bag', 
                   (50, 160), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 1)
        
        # Punch counters
        counter_y = 220
        
        # Left
        cv2.rectangle(data_window, (50, counter_y), (550, counter_y + 150), (255, 0, 255), 3)
        cv2.putText(data_window, 'LEFT CONTACTS', (150, counter_y + 40), 
                   cv2.FONT_HERSHEY_SIMPLEX, 1.0, (255, 255, 255), 2)
        cv2.putText(data_window, str(self.punch_counts['left']), (250, counter_y + 120), 
                   cv2.FONT_HERSHEY_SIMPLEX, 4.0, (255, 100, 255), 8)
        
        # Right
        cv2.rectangle(data_window, (600, counter_y), (1100, counter_y + 150), (255, 0, 255), 3)
        cv2.putText(data_window, 'RIGHT CONTACTS', (700, counter_y + 40), 
                   cv2.FONT_HERSHEY_SIMPLEX, 1.0, (255, 255, 255), 2)
        ...
        cv2.putText(data_window, 'TOTAL CONTACTS', (420, total_y + 50), 
    ...
    def run(self):
        ...
        print("\n" + "="*60)
        print("HEAVY BAG TRAINER - CONTACT DETECTION")
        print("="*60)
        print("FEATURES:")
        print("• Automatically detects heavy bag in frame")
        print("• Only counts punches that contact the bag")
        print("• Eliminates ALL false positives from general movement")
        print("• Green circles = hand contacting bag")
        print("• Red circles = hand not contacting bag")
        print("")
        print("CONTROLS:")
    ...
    def detect_bag(self, frame: np.ndarray) -> tuple:
        ...
        try:
            ...
            if contours:
                ...
                if area > 5000:  # Minimum area for heavy bag
...
def main():
    """Main entry point."""
    try:
        trainer = HeavyBagTrainer(camera_id=0)
        trainer.run()
    except Exception as e:
